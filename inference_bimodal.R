...
load("gam_simulation_bimodal.RData")
...
gam_objects <- parLapply(cl, iteration_list, function(i) {
  ...
  # Sampling random steps from an empirical movement
  ...
  MSE_fun <- function(time, model) {
    ...
    ssf_values <- exp(1.5 * quadrature_points$x1 - quadrature_points$x2 -
      0.02 * quadrature_points$cen) * my_fun_sl(ta_sl[, 2]) *
...
save.image(file = "inference_bimodal.RData")
