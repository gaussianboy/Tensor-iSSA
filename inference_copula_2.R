...
load("gam_simulation_copula_2.RData")
...
gam_objects = parLapply(cl, iteration_list, function(i){
  ...
  # rsa_df <- random_points(my_df, n = 5) %>% extract_covariates(landscape)
  
  # rsa_df$count <- as.numeric(rsa_df$case_)
  
  # issa_df$step_id_ <- as.factor(issa_df$step_id_)
  ...
  # issa_df_test$step_id_ <- as.factor(issa_df_test$step_id_)
  ...
  MSE_fun = function(time, model){
    ...
    ssf_values = exp(1.5*quadrature_points$x1 - quadrature_points$x2 -
                       0.02*quadrature_points$cen) *
      djoint(x = ta_sl, copula = cop, marginal_1 = marginal_1, marginal_2 =  marginal_2)
  ...
  return(list(issa_model_classic, issa, issa_x2, issa_x2_spatial,
...
save.image(file = "inference_copula_2.RData")
